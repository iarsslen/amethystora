...
5. `06-branding.sh` - Replaces the Bluefin branding from projectbluefin/common with Amethyst (artwork is generated by `node branding/generate.mjs`)
6. `08-firmware.sh` - Firmware configurations
7. `17-cleanup.sh` - Cleanup operations
8. `18-workarounds.sh` - Temporary fixes/workarounds
9. `19-initramfs.sh` - Regenerates initramfs
