...
This document provides essential information for coding agents working with the Amethyst repository to minimize exploration time and avoid common build failures.

## Repository Overview

**Amethyst** is a cloud-native desktop operating system, based on Bluefin and maintained by Arsslen Idadi (@iarsslen). It is an OS built on Fedora Linux using container technologies with atomic updates.

- **Type**: Container-based Linux distribution build system (75MB total, 74MB system files)
- **Base**: `ghcr.io/ublue-os/silverblue-main` with GNOME Shell and GDM removed, replaced by Hyprland (`sdegler/hyprland` COPR) and DankMaterialShell (`avengemedia/dms` + `avengemedia/danklinux` COPRs) with the DMS greeter on greetd
...
just build amethyst latest main "" "" "" "6.10.10-200.fc40.x86_64"
```
...
just validate amethyst latest main
```
...
2. **Stage `base`** (FROM silverblue-main): Base Amethyst image
...
- `IMAGE_NAME` - Target image name (amethyst/amethyst-dx)
...
4. `04-hyprland.sh` - Removes GNOME Shell/GDM, installs Hyprland, DankMaterialShell and the DMS greeter, builds the hyprbars title bar plugin against the installed Hyprland (bump `HYPRLAND_PLUGINS_COMMIT` with each Hyprland release), and configures the session
5. `05-override-install.sh` - Overrides base image packages
6. `06-branding.sh` - Replaces the Bluefin branding from projectbluefin/common with Amethyst (artwork is generated by `node branding/generate.mjs`)
7. `07-debrand.sh` - Renames every remaining Bluefin / Universal Blue file, command, service and reference to Amethyst (logic in `build_files/shared/debrand.py`; `20-tests.sh` fails the build if any is left)
8. `08-firmware.sh` - Firmware configurations
9. `17-cleanup.sh` - Cleanup operations
10. `18-workarounds.sh` - Temporary fixes/workarounds
11. `19-initramfs.sh` - Regenerates initramfs
...
- This project is published at iarsslen/amethyst; upstream Bluefin documentation lives in ublue-os/bluefin-docs
