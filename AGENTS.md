...
6. `06-branding.sh` - Replaces the Bluefin branding from projectbluefin/common with Amethystora (artwork is generated by `node branding/generate.mjs`)
7. `07-debrand.sh` - Renames every remaining Bluefin / Universal Blue file, command, service and reference to Amethystora (logic in `build_files/shared/debrand.py`; `20-tests.sh` fails the build if any is left)
